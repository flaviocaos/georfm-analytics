...
const AREAS = [
  {
    ...
    csvCols:"area_id,data_ocorrencia,intensidade",
    csvCols2:"area_id,data_ocorrencia,intensidade,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"area_id,data_analise,indice",
    csvCols2:"area_id,data_analise,indice,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"talhao_id,data_colheita,produtividade",
    csvCols2:"talhao_id,data_colheita,produtividade,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"municipio_id,data_notificacao,casos",
    csvCols2:"municipio_id,data_notificacao,casos,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"aoi_id,data_passagem,indice",
    csvCols2:"aoi_id,data_passagem,indice,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"estacao_id,data_medicao,vazao",
    csvCols2:"estacao_id,data_medicao,vazao,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"ativo_id,data_ocorrencia,custo",
    csvCols2:"ativo_id,data_ocorrencia,custo,latitude,longitude",
  ...
  },
  {
    ...
    csvCols:"setor_id,data_atendimento,vulnerabilidade",
    csvCols2:"setor_id,data_atendimento,vulnerabilidade,latitude,longitude",
...
function detectCols(rows) {
  ...
  const valCol = keys.find(k => !k.includes("id") && !k.includes("data") && !k.includes("date") && !k.includes("lat") && !k.includes("lon") && !k.includes("lng")) || keys[2];
...
function buildClients(rows, cols, k=5) {
  ...
  rows.forEach(r => {
    const id = r[idCol], d = new Date(r[dateCol]);
    if (!id || isNaN(d)) return;
  ...
  let arr = Object.values(map).map(c => ({
    ...c,
    rec: Math.floor((now-c.last)/864e5),
    mon: parseFloat(c.mon.toFixed(4))
  }));
...
function GeoMap({clients,area}){
    ...
    return(
      <div style={{background:C.bg3,borderRadius:10,padding:32,textAlign:"center",border:`1px dashed ${C.border2}`}}>
        ...
        <div style={{fontSize:11,color:C.text2,lineHeight:1.7,maxWidth:400,margin:"0 auto"}}>
          Seu CSV não contém colunas de coordenadas geográficas.<br/>
          Para visualizar no mapa, adicione as colunas <strong style={{color:area.color}}>latitude</strong> e <strong style={{color:area.color}}>longitude</strong> ao seu arquivo.
        </div>
  ...
  return(
    <div style={{borderRadius:10,overflow:"hidden",border:`1px solid ${C.border}`}}>
      <MapContainer center={[centerLat,centerLon]} zoom={6} style={{height:400,width:"100%"}} zoomControl={false}>
        <TileLayer
        ...
        {withCoords.map(c=>(
          <CircleMarker
            ...
            center={[c.lat,c.lon]}
            radius={6+c.freq*0.5}
            fillColor={colors[c.segIdx]||area.color}
            color={colors[c.segIdx]||area.color}
            weight={2}
            opacity={0.9}
            fillOpacity={0.7}
          >
            <Popup>
              <div style={{fontFamily:"system-ui",fontSize:12,minWidth:160}}>
                ...
                <div style={{marginBottom:4}}><strong>Segmento:</strong> {area.segmentos[c.segIdx]}</div>
                <div style={{marginBottom:2}}><strong>Recência:</strong> {c.rec} dias</div>
                <div style={{marginBottom:2}}><strong>Frequência:</strong> {c.freq}x</div>
                <div style={{marginBottom:2}}><strong>Valor:</strong> {fK(c.mon)}</div>
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",color:C.text}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"0 24px 48px"}}>
        ...
        <div style={{fontSize:12,color:C.text2,textAlign:"center",marginBottom:20}}>Cada área adapta automaticamente os campos RFM e os segmentos de classificação</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>
          {AREAS.map(area=>(
            <div key={area.id} onClick={()=>onSelect(area)}
              ...
              <div style={{fontSize:13,fontWeight:700,marginBottom:4,color:C.text}}>{area.label}</div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{width:"100%",maxWidth:580,display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
            ...
            <div style={{fontSize:11,color:C.text2,marginBottom:4}}>Formato mínimo: <code style={{color:area.color}}>{area.csvCols}</code></div>
            <div style={{fontSize:10,color:C.text3}}>Com mapa: <code style={{color:area.color+"99"}}>{area.csvCols2}</code></div>
          </div>
          <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px"}}>
            <div style={{fontSize:11,fontWeight:700,color:area.color,marginBottom:8}}>🎯 Segmentos que serão gerados:</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {area.segmentos.map((s,i)=>(
                <span key={s} style={{padding:"3px 10px",borderRadius:12,fontSize:11,background:`${area.color}${18-i*2}`,border:`1px solid ${area.color}44`,color:area.color,fontWeight:600}}>
                  {["⭐","✅","📊","⚠️","🔴"][i]} {s}
                </span>
              ))}
            </div>
          ...
          <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 16px"}}>
            <div style={{fontSize:10,color:C.text3,lineHeight:1.7}}>
              ✅ Qualquer quantidade de registros · ✅ Múltiplos registros por área · ✅ Com ou sem coordenadas · ✅ Encoding UTF-8
            </div>
          </div>
        </div>
      </div>
...
// ── Dashboard ──
function Dashboard({clients,area,csvName,hasCoords,onNewFile,onBack}){
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"system-ui,sans-serif",background:C.bg,color:C.text,fontSize:13}}>
      <div style={{width:sideOpen?220:60,flexShrink:0,background:C.bg2,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",transition:"width .25s",overflow:"hidden"}}>
        ...
        {sideOpen&&<div style={{padding:"10px 14px",borderBottom:`1px solid ${C.border}`,background:`${area.color}10`}}>
          ...
          <div style={{fontSize:9,color:C.text3,marginTop:2,wordBreak:"break-all"}}>{csvName}</div>
        </div>}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:C.bg2,borderBottom:`1px solid ${C.border}`,padding:"0 18px",height:52,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:10,color:C.text3}}>{clients.length} áreas · {area.label} {hasCoords?"· 🗺️ Mapa disponível":""}</div>
          </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16}}>
          ...
          {/* Mapa */}
          {page==="mapa"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{...card,borderLeft:`3px solid ${area.color}`}}>
              ...
              <div style={{fontSize:10,color:C.text3,marginBottom:12}}>
                {hasCoords?`${clients.filter(c=>c.lat&&c.lon).length} áreas com coordenadas · Tamanho do ponto = Frequência`:"Adicione lat/lon ao CSV para visualizar no mapa"}
              </div>
...
export default function App(){
  ...
  const handleFile=(txt,name)=>{
    ...
    try{
      ...
      if(!rows.length){setError("CSV sem dados válidos.");setLoading(false);return;}
      ...
      setClients(built);setCsvName(name);setHasCoords(coords);
    }catch(e){setError("Erro ao processar CSV.");}
    setLoading(false);
  };

  const handleFileInput=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>handleFile(ev.target.result,f.name);r.readAsText(f);};
  ...
  if(!clients.length)return(
    <div>
      ...
      {error&&<div style={{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",background:C.red,color:"#fff",padding:"10px 20px",borderRadius:10,fontSize:12,fontWeight:600}}>{error}</div>}
    </div>
  );
  return <Dashboard clients={clients} area={area} csvName={csvName} hasCoords={hasCoords} onNewFile={()=>setClients([])} onBack={()=>{setArea(null);setClients([]);setCsvName("");setHasCoords(false);}}/>;
}
